...
input_range = (0.1, 2)
output_range = get_range(generate_linear_dataset_of(target_function, n_inputs, n_outputs, dataset_size*20, input_range)[1])
...
inputs, outputs = generate_dataset_of(target_function, n_inputs, n_outputs, dataset_size, input_range)
#inputs = trigonometric_feature_expressivity(inputs, N)
norm_inputs = normalize_data(inputs, get_range(inputs), in_norm_range)
...
sorted_inputs, sorted_outputs = norm_inputs, norm_outputs
print_dataset(sorted_inputs, sorted_outputs)

# 2. Build the QNN and train it with the generated dataset
qnn = build_and_train_model(model_name, N, layers, n_inputs, n_outputs, observable_modes, observable_types, is_input_reupload, [sorted_inputs, sorted_outputs])
...
test_inputs, test_outputs = generate_linear_dataset_of(target_function, n_inputs, n_outputs, testing_set_size, input_range)
#test_inputs = trigonometric_feature_expressivity(test_inputs, N)
norm_test_inputs = normalize_data(test_inputs, get_range(test_inputs), in_norm_range)
