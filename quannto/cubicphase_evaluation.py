...
for (N, l, ph_add, in_norm_range, out_norm_range) in zip(modes, layers, photon_additions, in_norm_ranges, out_norm_ranges):
    ...
    # Load the QONN model and evaluate it
    #qnn = QNN.load_model("models/"+model_name+".txt")
    with open(f"losses/{model_name}.npy", "rb") as f:
    ...
    with open(f"testing/{model_name}.npy", "rb") as f:
...
legend_labels = [f'N={N}, L={l}, a† in modes {np.array(ph_add) + 1}' for (N, l, ph_add) in zip(modes, layers, photon_additions)]
